...
const noteFilePath = 'bin/noteServer/notes.txt';
int count = 0;
...
Future handlePost(HttpRequest request) async {
  ...
  try {
    decoded = await utf8.decoder.bind(request).transform(json.decoder).first as Map;
  } catch (e) {
...
void saveNote(HttpRequest request, String myNote) {
  try {
    File(noteFilePath).writeAsStringSync(myNote, mode: FileMode.append);
  } catch (e) {
